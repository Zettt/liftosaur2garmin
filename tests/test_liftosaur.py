    ...
    def test_requires_api_key(self, tmp_path: Path, monkeypatch) -> None:
        ...
        with patch.dict("os.environ", {}, clear=True):
            with pytest.raises(ValueError, match="API key required"):
                LiftosaurClient(api_key="")

    def test_accepts_api_key_param(self) -> None:
        client = LiftosaurClient(api_key="test-key-123")
...
class TestAPICalls:
    def test_get_workout_count(self) -> None:
        client = LiftosaurClient(api_key="test")
...
class TestRateLimiting:
    def test_delay_between_calls(self) -> None:
        ...
        with patch.object(client.session, "get", return_value=mock_resp):
            with patch("liftosaur2garmin.liftosaur.time.sleep") as mock_sleep:
                client._get("/test")
